...
class JsonFormatter:
    ...
    def process(self, data, x_column=None, y_columns=None):
        ...
        """
        logger.debug(
            f"Processing JSON data with x_column={x_column}, y_columns={y_columns}")
        ...
        if isinstance(data, str):
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Failed to load JSON from file {data}: {e}")
    ...
    def _json_to_dataframe(self, data):
        ...
        if isinstance(data, list):
            ...
            if any(isinstance(val, list) for val in data.values()):
                ...
                array_values = {k: v for k,
                                v in data.items() if isinstance(v, list)}
                ...
                if all(l == lengths[0] for l in lengths):
                    ...
                    logger.warning(
                        "JSON has uneven array lengths, normalizing")
