...
def discover_tools() -> Dict[str, Dict[str, Any]]:
    ...
    tools = get_all_tools()
    logger.info(f"Discovered {len(tools)} tools across {len(imported_modules)} modules")
    return tools
    ...
    for module_name in modules:
        ...
        for cls in service_classes:
            ...
            try:
                ...
                logger.error(f"Failed to initialize service {cls.__name__}: {e}")
    
    logger.info(f"Initialized {services_initialized} services of type {service_class.__name__}")
