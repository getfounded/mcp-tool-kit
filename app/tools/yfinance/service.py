...
class YFinanceService(ToolServiceBase):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.throttle_calls = int(self.get_env_var(
                "YFINANCE_THROTTLE_CALLS", default="5"))
            self.throttle_period = float(self.get_env_var(
                "YFINANCE_THROTTLE_PERIOD", default="60"))

            # Create rate limiter
            self.create_rate_limiter(
                "yfinance_api", self.throttle_calls, self.throttle_period)

            self.logger.info(
                f"YFinance service initialized with rate limit: {self.throttle_calls} calls per {self.throttle_period}s")
