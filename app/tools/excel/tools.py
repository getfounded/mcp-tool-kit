...
def xlsx_filter_dataframe(
    ...
    if query:
        ...
        filtered_df = self.filter_dataframe(
            df, column=column, value=value, operator=operator)
    else:
...
def xlsx_describe_dataframe(
    ...
    result = self.describe_dataframe(
        df, include=include, exclude=exclude, percentiles=percentiles)
...
def xlsx_dataframe_to_excel(
    ...
    # Export to Excel
    result = self.dataframe_to_excel(
        df, filename, sheet_name=sheet_name, index=index)
...
def xlsx_dataframe_to_csv(
    ...
    # Export to CSV
    result = self.dataframe_to_csv(
        df, filename, index=index, encoding=encoding, sep=sep)
