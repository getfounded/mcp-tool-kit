...
from app.tools.filesystem.service import FilesystemService, FilesystemSecurity
...
class TestFilesystemTools:
    ...
    async def test_list_directory(self):
        ...
        test_txt = next(
            (entry for entry in result["entries"] if entry["name"] == "test.txt"), None)
        assert test_txt is not None
        assert test_txt["type"] == "file"

        subdir = next(
            (entry for entry in result["entries"] if entry["name"] == "subdir"), None)
