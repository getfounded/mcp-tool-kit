...
def test_fred_service():
    ...
    from app.tools.fred.service import FREDService
    ...
    try:
        ...
        logger.info(
            f"FRED service initialization: {'SUCCESS' if result else 'FAILED'}")
...
def test_yfinance_service():
    ...
    try:
        result = service.initialize()
        logger.info(
            f"YFinance service initialization: {'SUCCESS' if result else 'FAILED'}")
    except Exception as e:
...
def test_excel_service():
    ...
    from app.tools.excel.service import ExcelService
    ...
    try:
        result = service.initialize()
        logger.info(
            f"Excel service initialization: {'SUCCESS' if result else 'FAILED'}")
    except Exception as e:
...
def test_brave_search_service():
    ...
    try:
        result = service.initialize()
        logger.info(
            f"Brave Search service initialization: {'SUCCESS' if result else 'FAILED'}")
    except Exception as e:
...
def test_filesystem_service():
    ...
    from app.tools.filesystem.service import FilesystemService
    ...
    try:
        result = service.initialize()
        logger.info(
            f"Filesystem service initialization: {'SUCCESS' if result else 'FAILED'}")
    except Exception as e:
...
def test_time_service():
    ...
    from app.tools.time.service import TimeService
    ...
    try:
        result = service.initialize()
        logger.info(
            f"Time service initialization: {'SUCCESS' if result else 'FAILED'}")
    except Exception as e:
...
def test_toolkit_client():
    ...
    logger.info(
        f"Toolkit client found {len(categories)} categories: {categories}")
...
def test_search_functions(toolkit):
    ...
    for query in queries:
        ...
        for i, result in enumerate(search_results[:3]):
            logger.info(
                f"  - {result['name']} ({result.get('category', 'Uncategorized')})")
